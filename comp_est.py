...
plt.suptitle("Data rank estimation",fontsize=22)
axes[0].set_title("PPCA cross-validated",fontsize=22)
this_meas = results["sep"]["pca_cv"]
mn = np.mean(this_meas,axis=0)
axes[0].plot(sep_comp_nums,mn,color="blue",linewidth=5,label="Separate")
this_meas = results["tog"]["pca_cv"]
mn = np.mean(this_meas,axis=0)
axes[0].plot(tog_comp_nums,mn,color="red",linewidth=5,label="Together")
axes[0].tick_params(axis="both",which="major",labelsize=22)
axes[0].set_ylabel("Log-likelihood",fontsize=22)
axes[0].set_xlabel("Number of components",fontsize=22)

axes[1].set_title("Scree plot",fontsize=22)
this_meas = results["sep"]["pca_screes"]
mn = np.mean(this_meas,axis=0)
axes[1].plot(np.arange(sep_comp_nums[-1]),mn,color="blue",linewidth=5,label="Separate")
axes[1].axhline(1)
axes[1].set_ylim(0,5)
...
mn = np.mean(this_meas,axis=0)
axes[1].plot(np.arange(tog_comp_nums[-1]),mn,color="red",linewidth=5,label="Together")
axes[1].axhline(1)
axes[1].set_ylim(0,5)
axes[1].legend(prop={"size":22})
axes[1].tick_params(axis="both",which="major",labelsize=22)
axes[1].set_ylabel("Eigenvalues",fontsize=22)
axes[1].set_xlabel("Number of components",fontsize=22)
